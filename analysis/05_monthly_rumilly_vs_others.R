make_monthly_rumilly_vs_others <- function(all_data, plot_dir, stat = c("mean", "median")) {
  ...
  agg_tbl <- aggregate(
    prix_m2 ~ month + city_group + Type.local,
    data = d,
    FUN = FUN
  )
  ...
  p <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = paste0("Monthly ", stat, " €/m²: Rumilly vs Other Cities"),
      x = "Month",
      y = paste0(stat, " €/m²"),
      color = "City group",
  ...
  out_file <- file.path(plot_dir, paste0("05_monthly_rumilly_vs_others_", stat, ".png"))
